...
import me.cumhax.aehack.mod.setting.EnumConverter;
...
public class ConfigManager implements Util {
    ...
    public String config = "aehackplus/config/";
    ...
    public static void setValueFromJson(Feature feature, Setting setting, JsonElement element) {
        ...
        ae.LOGGER.error("Unknown Setting type for: " + feature.getName() + " : " + setting.getName());
    }
    ...
    public void loadConfig(String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("aehackplus").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        if (files.contains(new File("aehackplus/" + name + "/"))) {
            this.config = "aehackplus/" + name + "/";
        } else {
            this.config = "aehackplus/config/";
        }
    ...
    public boolean configExists(String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("aehackplus").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        return files.contains(new File("aehackplus/" + name + "/"));
    }
    ...
    public void saveCurrentConfig() {
        File currentConfig = new File("aehackplus/currentconfig.txt");
        try {
            if (currentConfig.exists()) {
                FileWriter writer = new FileWriter(currentConfig);
                String tempConfig = this.config.replaceAll("/", "");
                writer.write(tempConfig.replaceAll("aehackplus", ""));
                writer.close();
            } else {
    ...
    private void loadPath(Path path, Feature feature) throws IOException {
        ...
        try {
            ...
            ae.LOGGER.error("Bad Config File for: " + feature.getName() + ". Resetting...");
    ...
    public JsonObject writeSettings(Feature feature) {
        ...
        for (Setting<?> setting : feature.getSettings()) {
            ...
            if (setting.isStringSetting()) {
                ...
                ((Setting<String>) setting).setValue(str.replace(" ", "_"));
            }
            try {
    ...
    public String getDirectory(Feature feature) {
        String directory = "";
        if (feature instanceof Module) directory = directory + ((Module) feature).getCategory().getName() + "/";
        return directory;
    }
}
