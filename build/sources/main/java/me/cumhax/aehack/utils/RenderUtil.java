...
public class RenderUtil implements Util {
    ...
    public static Vec3d to2D(double x, double y, double z) {
        ...
        if (result) return new Vec3d(screenCoords.get(0), (float) Display.getHeight() - screenCoords.get(1), screenCoords.get(2));
        return null;
    }
    ...
    public static void drawBoxESP(BlockPos pos, Color color, boolean secondC, Color secondColor, float lineWidth, boolean outline, boolean box, int boxAlpha, boolean air) {
        if (box) RenderUtil.drawBox(pos, new Color(color.getRed(), color.getGreen(), color.getBlue(), boxAlpha));
        if (outline) RenderUtil.drawBlockOutline(pos, secondC ? secondColor : color, lineWidth, air);
    }
    ...
    public static void drawText(BlockPos pos, String text) {
        ...
        GlStateManager.disableDepth();
        GlStateManager.translate(-((double) ae.textManager.getStringWidth(text) / 2.0), 0.0, 0.0);
        ae.textManager.drawStringWithShadow(text, 0.0f, 0.0f, -5592406);
        GlStateManager.popMatrix();
    }
